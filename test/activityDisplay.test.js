...
test("football keeps distance and duration but hides pace and elevation", () => {
  ...
  assert.equal(metrics.secondaryLabel, "Puls");
...
test("strength and mobility use duration as the main metric", () => {
  ...
  assert.equal(metrics.primaryLabel, "Dauer");
...
test("running still shows pace, elevation and pulse", () => {
  ...
  assert.equal(metrics.secondaryPrimary, "+113 m");
  assert.equal(metrics.primaryLabel, "Distanz");
  assert.equal(metrics.secondaryLabel, "Höhenmeter");
