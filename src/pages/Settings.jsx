...
export default function Settings() {
  const { state, setState, session, cloudStatus, cloudUpdatedAt, cloudError, localStorageMode, localStorageError, imageStorageStatus, imageStorageMessage, retryImageMigration, calendarToken, intervalsSyncStatus, syncIntervalsNow, uploadLocalState, reloadCloudState, logout } = useApp();
  ...
  async function checkCalendarFeed() {
    ...
    try {
      ...
      const preview = Array.isArray(status.sampleEvents) && status.sampleEvents.length
        ? ` · erster Termin ${status.sampleEvents[0].date}: ${status.sampleEvents[0].title}`
        : "";
      setCalendarMessage(`Abo-Feed erreichbar ✓ · Feed ${status.feedVersion || "aktuell"} · ${Number(status.eventCount || 0)} Kalendertermine · Cloud-Stand ${updated}${preview}`);
    } catch (error) {
  ...
  return <>
    ...
    {section === "connections" && <div className="grid">
      <Card className="wide"><p className="eyebrow">Endurance Intelligence Cloud</p><h2>Geräteübergreifend synchronisiert</h2><p className="muted">Angemeldet als <b>{session?.user?.email}</b>. Änderungen werden automatisch in Supabase gespeichert.</p><span className={`cloud-status ${cloudStatus}`}>{cloudStatusLabel}</span>{cloudUpdatedAt && <p className="muted">Letzte Cloud-Aktualisierung: {new Date(cloudUpdatedAt).toLocaleString("de-DE")}</p>}{cloudError && <p className="connection-message cloud-error-message">{cloudError}</p>}{localStorageMode !== "full" && <p className={`connection-message ${localStorageMode === "error" ? "cloud-error-message" : ""}`}>{localStorageMode === "compact" ? "Mobiler Speicher war knapp. EI hat Bilder aus dem lokalen Cache entfernt; Reviews und Plan bleiben lokal gesichert." : localStorageMode === "recovery" ? "Mobiler Speicher ist voll. EI hält deshalb mindestens Reviews, Plan und Konfiguration in einer Notfall-Sicherung fest." : `Lokale Sicherung fehlgeschlagen: ${localStorageError || "Browserspeicher nicht verfügbar."}`}</p>}<div className="button-row"><button onClick={cloudStatus === "error" ? reloadCloudState : uploadLocalState}>{cloudStatus === "conflict" ? "Lokalen Stand behalten" : cloudStatus === "error" ? "Cloud erneut verbinden" : "Lokale Daten in Cloud übernehmen"}</button><button className="secondary" onClick={reloadCloudState}>{cloudStatus === "conflict" ? "Neueren Cloud-Stand laden" : "Cloud neu laden"}</button><button className="secondary" onClick={logout}>Abmelden</button></div>{cloudStatus === "error" && <p className="muted">Lokale Änderungen werden unabhängig von der Cloud weiter gesichert. Erst nach erfolgreichem Laden wird wieder automatisch in die Cloud geschrieben.</p>}</Card>
    ...
    {section === "data" && <div className="grid">
      ...
      <Card><p className="eyebrow">Apple Kalender</p><h2>Kalender-Abo</h2><p className="muted">Die Cloud-Adresse liefert deinen aktuellen Wochenplan als echte .ics-Abo-Quelle. v3.9.102 verwendet dafür bewusst eine neue <code>feed.ics</code>-Adresse, damit ein bereits von Apple festgehaltener alter Feed nicht weiterverwendet wird.</p><div className="button-row"><button onClick={() => navigator.clipboard?.writeText(calendarUrl).then(() => setCalendarMessage("Neue Apple-Abo-Adresse kopiert. Das bestehende Endurance-Intelligence-Abo einmal entfernen und mit dieser Adresse neu hinzufügen."))} disabled={!calendarToken}>Neue Apple-Abo-Adresse kopieren</button><button className="secondary" onClick={checkCalendarFeed} disabled={!calendarToken || calendarBusy}>{calendarBusy ? "Prüfe Feed …" : "Abo-Feed prüfen"}</button><button className="secondary" onClick={() => downloadCalendar(state.plan)}>ICS als Datei</button></div>{calendarUrl && <><label className="calendar-url-label">Neue Abo-Adresse<input readOnly value={calendarUrl} onFocus={(event) => event.target.select()} /></label><div className="setup-note"><strong>Einmalig nach diesem Fix:</strong> Das bisherige Endurance-Intelligence-Abo in Apple Kalender entfernen und mit dieser neuen <code>feed.ics</code>-Adresse erneut hinzufügen. Danach bleibt die Adresse stabil. Apple bestimmt selbst, wann ein iCloud-Kalenderabo erneut abgerufen wird; die 15-Minuten-Angabe im Feed ist deshalb nur ein Refresh-Hinweis und keine garantierte Abrufzeit.</div><p className="muted">iPhone/iPad: Kalender → Kalender → Hinzufügen → Kalenderabonnement hinzufügen → Adresse einsetzen → bei Account <strong>iCloud</strong> auswählen.</p></>}{calendarMessage && <p className="connection-message">{calendarMessage}</p>}</Card>
